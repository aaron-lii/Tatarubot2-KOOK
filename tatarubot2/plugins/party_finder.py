...
async def party_finder_run(data_centre=""):
    ...
    print(len(data_centre_list), len(description_list), len(meta_list), len(duty_list))
    for i in range(len(data_centre_list)):
        ...
        text_now = "{} ".format(full_index)
        text_now += "" + data_centre_now + " "
        text_now += duty_now + "\n"
        # text_now += "---\n"
        text_now += "(font)" + description_now + "(font)[warning]\n"
        # text_now += "---\n"
    ...
    off_set = 30
    # cards = []
    for i in range(0, len(text_list), off_set):
        final_text = "**(font)【 招 募 板 】 {} ~ {}(font)[warning]**\n---\n".format(str(i + 1),
                                                                                   str(min(i + off_set, len(text_list))))
        final_text += "".join(text_list[i: i + off_set])


        res = [{"type": "section",
                "text": {"type": "kmarkdown", "content": final_text}, }]
        cards = [{"type": "card", "theme": "warning", "size": "lg", "modules": res}]
...
async def handle_item(bot: Bot, event: Event, state: T_State):
    await party_finder_run(state["party_finder_info"])

    await party_finder.finish()