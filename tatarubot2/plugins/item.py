...
async def run(name, bot: Bot):
    ...
    try:
        ...
        if isinstance(res_data, dict):
            ...
            img_b = base64.b64decode(res_data[0][9:])
            img_name = str(int(time.time() * 10))
            img_url = await bot.upload_file(img_b, img_name)
            # print(img_url)
            msg = Message([MessageSegment(type="image", data={"file_key": img_url}),
                           MessageSegment(type="text", data={"content": "\n".join(res_data[1:])})])
