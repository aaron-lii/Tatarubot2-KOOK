...
from nonebot.adapters.kaiheila import Bot, Event, MessageSegment
...
async def run(args):
    ...
    if not result_list:
        res = [{"type": "section",
                "text": {"type": "kmarkdown", "content": "没空房子了"}, }]
        cards = [{"type": "card", "theme": "info", "size": "lg", "modules": res}]

        await house.finish(MessageSegment.Card(cards))
        return
    else:
        off_set = 100
        for i in range(len(result_list) // off_set + 1):
            res_text = "\n".join(result_list[i * off_set: i * off_set + off_set])
            res = [{"type": "section",
                    "text": {"type": "kmarkdown", "content": res_text}, }]
            cards = [{"type": "card", "theme": "info", "size": "lg", "modules": res}]

            await house.send(MessageSegment.Card(cards))
