...
from nonebot.adapters.kaiheila import Bot, Event, Message, MessageSegment
...
async def run():
    ...
    week_ics.sort()
    feature_ics.sort()
    res = "**(font)今天是 " + str(time_info.date()).replace("-", ".") + "(font)[success]**\n"
    if warn_ics:
        res += "**(font)近2天结束(font)[danger]**\n"
        for item in warn_ics:
            res += res_format(item) + "\n"
    if week_ics:
        res += "**(font)近7天内(font)[warning]**\n"
        for item in week_ics:
            res += res_format(item) + "\n"
    if feature_ics:
        res += "**(font)未来活动(font)[primary]**\n"
    ...
    # res = {"type": "plain-text", "content": res}
    res = [{"type": "section",
            "text": {"type": "kmarkdown", "content": res},}]
    cards = [{"type": "card", "theme": "info", "size": "lg", "modules": res}]
    res = MessageSegment.Card(cards)
